...
export default function BirthdayWish() {
  ...
  const fetchEmployees = async () => {
    ...
    try {
      ...
      // Process and calculate birthday information
      ...
      const processed = (data || []).map((emp) => {
        ...
        if (bdayMonth === today.getMonth() && bdayDate === today.getDate()) {
          ...
          // If birthday has passed this year, roll over to next year
          if (nextBday < today) {
        ...
        // Current Age (before today's birthday if today, or current age as of right now)
        ...
        // Age to be completed on next birthday
        ...
        return {
          ...
          photo: emp.aadhar_front_photo || "", // Front Aadhar photo as fallback
          dateOfBirth: dob,
          isToday,
          remainingDays,
          currentAge,
  ...
  // Filter department list dynamically
  ...
  // Split into Today and Upcoming categories
  ...
  const upcomingBirthdays = useMemo(() => {
  return filteredEmployees
    .filter((emp) => !emp.isToday && emp.remainingDays <= 15) // 👈 Added this condition
    .sort((a, b) => a.remainingDays - b.remainingDays);
}, [filteredEmployees]);
  ...
  // Send WhatsApp wish via Edge Function with client-side fallback
  const sendWish = async () => {
    if (!selectedEmp) return;
    
    // Choose personal number first, fallback to family number
    ...
    try {
      ...
      if (funcErr) {
        ...
        } else if (funcErr.response && typeof funcErr.response.json === 'function') {
          try {
            const errBody = await funcErr.response.json();
            if (errBody && errBody.error) {
              displayError = errBody.error;
            }
          } catch (e) {}
      ...
      // Fallback: Show toast and redirect to WhatsApp Web with prefilled message
      toast.error(`Automated send failed: ${err.message || "Failed to trigger Edge Function"}. Redirecting to WhatsApp Web...`, {
        duration: 6000,
      });
      ...
      // Confetti effect trigger for fallback too since they still send it
      setConfettiActive(true);
      setTimeout(() => setConfettiActive(false), 3000);
      ...
      <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                Calculate, search, and send automated greetings to employees on their special days.
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-4 w-full md:w-auto">
            <div className="bg-rose-50/60 border border-rose-100/80 rounded-2xl px-5 py-3 flex-1 md:flex-initial min-w-[120px]">
              <div className="text-xs text-rose-500 font-bold uppercase tracking-wider">Today</div>
              <div className="text-2xl font-black text-rose-600 mt-1">
                {todaysBirthdays.length}
              </div>
            </div>
            <div className="bg-indigo-50/60 border border-indigo-100/80 rounded-2xl px-5 py-3 flex-1 md:flex-initial min-w-[120px]">
        ...
        <div className="space-y-10">
          ...
          <div className="space-y-4">
            ...
            {todaysBirthdays.length > 0 ? (
              ...
              /* Professional Empty State for Today's birthdays */
          ...
          <div className="space-y-4">
            ...
            {upcomingBirthdays.length > 0 ? (
              ...
              /* Empty state for upcoming birthdays */
            ...
            <div className="p-6 space-y-5 overflow-y-auto flex-1">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-[10px] text-slate-400 font-medium mt-1">
                  Automated dispatch configured: Invokes Supabase Edge Function. Falls back to direct chat redirect if needed.
                </p>
              </div>
