...
const Indent = () => {
  ...
  const visibleColumns = [
    'Company Name', 'Post', 'Gender', 'Department', 'Prefer', 'Experience',
  ...
  const fetchMasterData = async () => {
    try {
      const { data, error } = await supabase
        .from("master_hr")
        .select("department, social_site, firm_name");
      ...
      const companies = [
        ...new Set(data.map((item) => item.firm_name).filter(Boolean)),
      ];
  ...
  const fetchIndentData = async () => {
    try {
      ...
      const processedData = data.map((row) => ({
        ...
        companyName: row.company_name, // add this
        post: row.post,
        gender: row.gender,
          ...
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex justify-end space-x-2 pt-4">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-indigo-700 transition-all duration-200 flex items-center justify-center"
                  disabled={submitting}
                >
                  {submitting ? (
                    <>
      ...
      <div className="bg-white shadow-lg rounded-lg overflow-hidden">
        <div className="max-h-[calc(100vh-300px)] overflow-auto">
          <table className="min-w-full divide-y divide-gray-200 shadow text-nowrap">
            <thead className="text-white bg-indigo-600 sticky top-0 z-30 text-nowrap text-center">
              <tr>
                <th className="sticky left-0 z-40 text-white bg-indigo-600 px-6 py-3 text-xs font-medium  uppercase tracking-wider min-w-[160px]">
                  Progress
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-white bg-indigo-600  uppercase tracking-wider">
                  Indent Number
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-white bg-indigo-600 uppercase tracking-wider">
                  Company Name
                </th>
                <th className="px-6 py-3 text-center text-xs font-medium text-white bg-indigo-600  uppercase tracking-wider">
                  Designation
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-white bg-indigo-600  uppercase tracking-wider">
                  Gender
